...
tr.reorder();
tr.send<8>((char*)__xlx_apatb_param_a, 8);
}
#else
{
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<64> > a_pc_buffer(8);
          int i = 0;
          ...
          if (i > 0) {{
            int i = 0;
            for (int j = 0, e = 8; j < e; j += 1, ++i) {((char*)__xlx_apatb_param_a)[j*8+0] = a_pc_buffer[i].range(7, 0).to_int64();
