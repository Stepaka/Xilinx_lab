...
tr.reorder();
tr.send<4>((char*)__xlx_apatb_param_a, 16);
}
#else
{
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<32> > a_pc_buffer(16);
          int i = 0;
          ...
          if (i > 0) {{
            int i = 0;
            for (int j = 0, e = 16; j < e; j += 1, ++i) {((char*)__xlx_apatb_param_a)[j*4+0] = a_pc_buffer[i].range(7, 0).to_int64();
...
tr.import<4>((char*)__xlx_apatb_param_a, 16, 0);
  }
tr.reorder();
...
tr.import<4>((char*)__xlx_apatb_param_b, 16, 0);
  }
tr.reorder();
...
tr.import<4>((char*)__xlx_apatb_param_c, 16, 0);
  }
tr.reorder();
