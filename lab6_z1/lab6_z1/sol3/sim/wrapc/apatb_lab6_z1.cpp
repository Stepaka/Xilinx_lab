...
tr.reorder();
tr.send<2>((char*)__xlx_apatb_param_a, 32);
}
#else
{
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<16> > a_pc_buffer(32);
          int i = 0;
          ...
          if (i > 0) {{
            int i = 0;
            for (int j = 0, e = 32; j < e; j += 1, ++i) {((char*)__xlx_apatb_param_a)[j*2+0] = a_pc_buffer[i].range(7, 0).to_int64();
((char*)__xlx_apatb_param_a)[j*2+1] = a_pc_buffer[i].range(15, 8).to_int64();
}}}
        } // end transaction
      } // end file is good
...
tr.import<2>((char*)__xlx_apatb_param_a, 32, 0);
  }
tr.reorder();
...
tr.import<2>((char*)__xlx_apatb_param_b, 32, 0);
  }
tr.reorder();
...
tr.import<2>((char*)__xlx_apatb_param_c, 32, 0);
  }
tr.reorder();
aesl_fh.write(AUTOTB_TVIN_c, tr.p, tr.tbytes);
}

  tcl_file.set_num(32, &tcl_file.c_depth);
#else
...
aesl_fh.write(AUTOTB_TVIN_c, begin_str(AESL_transaction));
{
  ...
  if (__xlx_apatb_param_c) {
    for (int j = 0  - 0, e = 32 - 0; j != e; ++j) {
sc_bv<16> __xlx_tmp_lv = ((short*)__xlx_apatb_param_c)[j];
