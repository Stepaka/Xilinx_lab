...
tr.reorder();
tr.send<16>((char*)__xlx_apatb_param_r, 2);
}
#else
{
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<128> > r_pc_buffer(2);
          int i = 0;
          ...
          if (i > 0) {{
            int i = 0;
            for (int j = 0, e = 2; j < e; j += 1, ++i) {((long long*)__xlx_apatb_param_r)[j*2+0] = r_pc_buffer[i].range(63,0).to_int64();
((long long*)__xlx_apatb_param_r)[j*2+1] = r_pc_buffer[i].range(127,64).to_int64();
}}}
        } // end transaction
      } // end file is good
...
transaction<128> tr(2);
  __xlx_offset_byte_param_r = 0*16;
  if (__xlx_apatb_param_r) {
tr.import<16>((char*)__xlx_apatb_param_r, 2, 0);
  }
tr.reorder();
aesl_fh.write(AUTOTB_TVIN_r, tr.p, tr.tbytes);
}

  tcl_file.set_num(2, &tcl_file.r_depth);
#else
...
__xlx_tmp_lv.range(63,0) = ((long long*)__xlx_apatb_param_r)[j*2+0];
__xlx_tmp_lv.range(127,64) = ((long long*)__xlx_apatb_param_r)[j*2+1];
...
transaction<128> tr(2);
  __xlx_offset_byte_param_x = 0*16;
  if (__xlx_apatb_param_x) {
tr.import<16>((char*)__xlx_apatb_param_x, 2, 0);
  }
tr.reorder();
aesl_fh.write(AUTOTB_TVIN_x, tr.p, tr.tbytes);
}

  tcl_file.set_num(2, &tcl_file.x_depth);
#else
...
aesl_fh.write(AUTOTB_TVIN_x, begin_str(AESL_transaction));
{
  ...
  if (__xlx_apatb_param_x) {
    for (int j = 0  - 0, e = 2 - 0; j != e; ++j) {
sc_bv<128> __xlx_tmp_lv;
__xlx_tmp_lv.range(63,0) = ((long long*)__xlx_apatb_param_x)[j*2+0];
__xlx_tmp_lv.range(127,64) = ((long long*)__xlx_apatb_param_x)[j*2+1];
