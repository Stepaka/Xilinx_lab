...
{
transaction<32> tr(15);
aesl_fh.read(AUTOTB_TVOUT_PC_res, tr.p, tr.tbytes);
tr.reorder();
tr.send<4>((char*)__xlx_apatb_param_res, 15);
}
#else
{
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<32> > res_pc_buffer(15);
          int i = 0;
          ...
          if (i > 0) {{
            int i = 0;
            for (int j = 0, e = 15; j < e; j += 1, ++i) {((char*)__xlx_apatb_param_res)[j*4+0] = res_pc_buffer[i].range(7, 0).to_int64();
...
tr.import<4>((char*)__xlx_apatb_param_in_a, 15, 0);
  }
tr.reorder();
aesl_fh.write(AUTOTB_TVIN_in_a, tr.p, tr.tbytes);
}

  tcl_file.set_num(15, &tcl_file.in_a_depth);
#else
...
tr.import<4>((char*)__xlx_apatb_param_in_b, 15, 0);
  }
tr.reorder();
aesl_fh.write(AUTOTB_TVIN_in_b, tr.p, tr.tbytes);
}

  tcl_file.set_num(15, &tcl_file.in_b_depth);
#else
...
tr.import<4>((char*)__xlx_apatb_param_res, 15, 0);
  }
tr.reorder();
aesl_fh.write(AUTOTB_TVIN_res, tr.p, tr.tbytes);
}

  tcl_file.set_num(15, &tcl_file.res_depth);
#else
