...
transaction<16> tr(512);
  __xlx_offset_byte_param_d_in = 0*2;
  if (__xlx_apatb_param_d_in) {
tr.import<2>((char*)__xlx_apatb_param_d_in, 512, 0);
  }
tr.reorder();
aesl_fh.write(AUTOTB_TVIN_d_in, tr.p, tr.tbytes);
}

  tcl_file.set_num(512, &tcl_file.d_in_depth);
#else
...
sc_bv<16> __xlx_tmp_lv = ((short*)__xlx_apatb_param_d_in)[j];
