...
#define AUTOTB_TVOUT_d_in "../tv/cdatafile/c.lab5_z1.autotvout_d_in.dat"
// wrapc file define:
...
// tvout file define:
#define AUTOTB_TVOUT_PC_d_in "../tv/rtldatafile/rtl.lab5_z1.autotvout_d_in.dat"
// tvout file define:
...
static bool RTLOutputCheckAndReplacement(std::string &AESL_token, std::string PortName) {
  ...
  return err;}
extern "C" void lab5_z1_hw_stub_wrapper(volatile void *, volatile void *);
...
tr.reorder();
tr.send<2>((char*)__xlx_apatb_param_d_out, 128);
}
#else
{
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<16> > d_out_pc_buffer(128);
          int i = 0;
          ...
          if (i > 0) {{
            int i = 0;
            for (int j = 0, e = 128; j < e; j += 1, ++i) {((char*)__xlx_apatb_param_d_out)[j*2+0] = d_out_pc_buffer[i].range(7, 0).to_int64();
...
unsigned __xlx_offset_byte_param_d_in = 0;
#ifdef USE_BINARY_TV_FILE
{
aesl_fh.touch(AUTOTB_TVIN_d_in, 'b');
transaction<64> tr(128);
  __xlx_offset_byte_param_d_in = 0*8;
  if (__xlx_apatb_param_d_in) {
tr.import<8>((char*)__xlx_apatb_param_d_in, 128, 0);
  }
tr.reorder();
aesl_fh.write(AUTOTB_TVIN_d_in, tr.p, tr.tbytes);
}

  tcl_file.set_num(128, &tcl_file.d_in_depth);
#else
// print d_in Transactions
{
aesl_fh.write(AUTOTB_TVIN_d_in, begin_str(AESL_transaction));
{
  __xlx_offset_byte_param_d_in = 0*8;
  if (__xlx_apatb_param_d_in) {
    for (int j = 0  - 0, e = 128 - 0; j != e; ++j) {
sc_bv<64> __xlx_tmp_lv = ((long long*)__xlx_apatb_param_d_in)[j];
aesl_fh.write(AUTOTB_TVIN_d_in, __xlx_tmp_lv.to_string(SC_HEX)+string("\n"));
    }
  }
}

  tcl_file.set_num(128, &tcl_file.d_in_depth);
aesl_fh.write(AUTOTB_TVIN_d_in, end_str());
}
...
tr.import<2>((char*)__xlx_apatb_param_d_out, 128, 0);
  }
tr.reorder();
aesl_fh.write(AUTOTB_TVIN_d_out, tr.p, tr.tbytes);
}

  tcl_file.set_num(128, &tcl_file.d_out_depth);
#else
...
lab5_z1_hw_stub_wrapper(__xlx_apatb_param_d_in, __xlx_apatb_param_d_out);
