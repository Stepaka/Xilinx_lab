...
transaction<32> tr(256);
  __xlx_offset_byte_param_d_in = 0*4;
  if (__xlx_apatb_param_d_in) {
tr.import<4>((char*)__xlx_apatb_param_d_in, 256, 0);
  }
tr.reorder();
aesl_fh.write(AUTOTB_TVIN_d_in, tr.p, tr.tbytes);
}

  tcl_file.set_num(256, &tcl_file.d_in_depth);
#else
...
sc_bv<32> __xlx_tmp_lv = ((int*)__xlx_apatb_param_d_in)[j];
