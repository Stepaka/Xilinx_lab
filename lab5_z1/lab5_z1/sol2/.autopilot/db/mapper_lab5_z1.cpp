...
extern "C" void lab5_z1(short*, short*);
extern "C" void apatb_lab5_z1_hw(volatile void * __xlx_apatb_param_d_in, volatile void * __xlx_apatb_param_d_out) {
  ...
  vector<sc_bv<16> >__xlx_d_in__tmp_vec;
  for (int j = 0, e = 512; j != e; ++j) {
    sc_bv<16> _xlx_tmp_sc;
    _xlx_tmp_sc.range(7, 0) = ((char*)__xlx_apatb_param_d_in)[j*2+0];
    _xlx_tmp_sc.range(15, 8) = ((char*)__xlx_apatb_param_d_in)[j*2+1];
    __xlx_d_in__tmp_vec.push_back(_xlx_tmp_sc);
  }
  int __xlx_size_param_d_in = 512;
  int __xlx_offset_param_d_in = 0;
  int __xlx_offset_byte_param_d_in = 0*2;
  short* __xlx_d_in__input_buffer= new short[__xlx_d_in__tmp_vec.size()];
  for (int i = 0; i < __xlx_d_in__tmp_vec.size(); ++i) {
    __xlx_d_in__input_buffer[i] = __xlx_d_in__tmp_vec[i].range(15, 0).to_uint64();
  }
...
// print __xlx_apatb_param_d_in
  sc_bv<16>*__xlx_d_in_output_buffer = new sc_bv<16>[__xlx_size_param_d_in];
  for (int i = 0; i < __xlx_size_param_d_in; ++i) {
    __xlx_d_in_output_buffer[i] = __xlx_d_in__input_buffer[i+__xlx_offset_param_d_in];
  }
  for (int i = 0; i < __xlx_size_param_d_in; ++i) {
    ((char*)__xlx_apatb_param_d_in)[i*2+0] = __xlx_d_in_output_buffer[i].range(7, 0).to_uint();
    ((char*)__xlx_apatb_param_d_in)[i*2+1] = __xlx_d_in_output_buffer[i].range(15, 8).to_uint();
  }
